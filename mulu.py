...
# 全局变量
INSTALL_DIR = Path.home() / ".mulu"  # 用户主目录下的隐藏文件夹，避免root权限
...
UUID = "2faaf996-d2b0-440d-8258-81f2b05dd0e4"                     # UUID，留空则自动生成
PORT = 49999                   # Vmess端口，留空或0则自动生成
DOMAIN = "newgg.xwm.dpdns.org"                   # 域名，留空则自动获取
TOKEN = "eyJhIjoiOGRmYzUzODAwYmQ5NGQwZjYzMDNkMGUzNzA4Y2IxMjUiLCJ0IjoiYjU2ZGRmZTAtOWM1Mi00ZGZiLThmMmQtNTlmYzEwNWI2ZWM5IiwicyI6Ik56RXdaakppWlRRdFlqWXdZUzAwTm1Zd0xXRmhOV1V0TlRrNE56ZGhZbU0yTUdOaSJ9"                 # Cloudflare Token，留空则用Quick Tunnel
# =========================================
...
def print_info():
    ...
    print("\033[36m│             \033[33m✨ zmkSB Python3 自定义域名版 ✨              \033[36m│\033[0m")
    print("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
...
def print_usage():
    ...
    print("  \033[36mpython3 script.py --agk YOUR_TOKEN\033[0m     - 使用zmk Tunnel Token安装")
    ...
    print("  \033[36mexport agk=YOUR_zmk_TUNNEL_TOKEN\033[0m       - 设置zmk Tunnel Token")
    print()
...
def generate_links(domain, port_vm_ws, uuid_str):
    ...
    list_content_color_file.append("\033[36m│                \033[33m✨ zmkSB 节点信息 ✨                   \033[36m│\033[0m")
    list_content_color_file.append("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
    ...
    print("\033[36m╭───────────────────────────────────────────────────────────────╮\033[0m")
    print("\033[36m│                \033[33m✨ zmkSB 安装成功! ✨                    \033[36m│\033[0m")
    print("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
...
def install(args):
    ...
    # zmk Tunnel Token (agk)
    zmk_token = args.agk or os.environ.get("agk") or TOKEN
    if not zmk_token:
        zmk_token_input = input("请输入 zmk Tunnel Token (AGK) (例如: eyJhIjo...Ifs9, 若使用Cloudflare Zero Trust隧道请输入, 留空则使用临时隧道): ").strip()
        zmk_token = zmk_token_input or None # None if empty
    if zmk_token:
        print(f"使用 zmk Tunnel Token: ******{zmk_token[-6:]}") # 仅显示末尾几位
        write_debug_log(f"zmk Token: Present (not logged for security)")
    else:
        print("未提供 zmk Tunnel Token，将使用临时隧道 (Quick Tunnel)。")
        write_debug_log("zmk Token: Not provided, using Quick Tunnel.")
    # Custom Domain (agn)
    ...
    elif zmk_token: # 如果用了token，必须提供域名
        print("\033[31m错误: 使用 zmk Tunnel Token 时必须提供自定义域名 (agn/--domain)。\033[0m")
        sys.exit(1)
    else:
    ...
    config_data = {
        ...
        "zmk_token": zmk_token, # Will be None if not provided
    ...
    if not zmk_token and not custom_domain: # Quick tunnel and no pre-set domain
        print("正在等待临时隧道域名生成...")
        ...
        if not final_domain:
            ...
            sys.exit(1)
    elif zmk_token and not custom_domain: # Should have exited earlier, but as a safeguard
...
def check_status():
    ...
    if sb_running and cf_running and LIST_FILE.exists():
        print("\033[36m╭───────────────────────────────────────────────────────────────╮\033[0m")
        print("\033[36m│                \033[33m✨ zmkSB 运行状态 ✨                    \033[36m│\033[0m")
        print("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
        ...
        if CUSTOM_DOMAIN_FILE.exists():
            ...
            elif not config.get("zmk_token") and LOG_FILE.exists(): # Quick tunnel, try log
                log_content = LOG_FILE.read_text()
                match = re.search(r'https://([a-zA-Z0-9.-]+\.trycloudflare\.com)', log_content)
                if match:
                    domain_to_display = match.group(1)
                    print(f"\033[36m│ \033[32mzmk临时域名: \033[0m{domain_to_display}")
...
def create_startup_script():
    ...
    zmk_token = config.get("zmk_token") # Safely get token, might be None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1: # 如果只运行脚本名，没有其他参数
        ...
        if INSTALL_DIR.exists() and CONFIG_FILE.exists() and SB_PID_FILE.exists() and zmk_PID_FILE.exists():
            print(f"\033[33m检测到 zmkSB 可能已安装。显示当前状态。\033[0m")
